...
#
# output org mode text, usable inside emacs for browsing/searching
#
# Copyright (c) 2016 T.v.Dein <tlinden |AT| cpan.org>.
# All Rights Reserved.        Std. disclaimer applies.
# Artistic License,     same as perl itself. Have fun.
#
# repo: https://github.com/TLINDEN/rezkonv2orgmode
...
foreach my $entry (@list) {
  ...
  foreach (split /\r?\n/, $entry) {
    if (/^\s*$/) {
      ...
      if ($nextform) {
        # formulation may contain paragraphs,
...
if ($ENV{DEBUGRK}) {
  ...
  #
  # actual output of parsed recipes
  if (! $catmode) {
	print "#-*- mode: org -*-\n#+STARTUP: hideall\n\n";
  }
